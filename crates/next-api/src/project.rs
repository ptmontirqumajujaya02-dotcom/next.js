...
impl Project {
    ...
    /// Root path scoping the aggregate server-HMR subscription to App Router
    /// entries only. Server HMR is an App Router-only feature; App Router entry
    /// chunks are emitted under `server/app/`, while Pages Router entries live
    /// under `server/pages/`. Both produce entry-chunk-list content, so scoping
    /// the root here keeps the aggregate subscription from also applying updates
    /// to Pages Router entries and sending them `SERVER_COMPONENT_CHANGES`,
    /// which would force a full page reload on the Pages client (losing Fast
    /// Refresh state and breaking error recovery).
    #[turbo_tasks::function]
    async fn aggregate_hmr_root_path(
        ...
        match target {
            HmrTarget::Client => bail!("aggregate server HMR is not implemented for the client"),
