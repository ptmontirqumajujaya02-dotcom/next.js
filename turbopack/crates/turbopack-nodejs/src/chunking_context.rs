...
impl NodeJsChunkingContext {
    ...
    /// `chunks`, without producing an evaluate chunk. Returns an empty list
    /// outside development (no HMR subscription exists then).
    ///
    ...
    pub async fn server_hmr_chunk_list(
        ...
        // Only meaningful in development; production has no HMR subscription.
        ...
        Ok(Vc::cell(vec![ResolvedVc::upcast(
            EcmascriptBuildNodeChunkList::new(*self, path, chunks)
                .to_resolved()
                .await?,
        )]))
    }
